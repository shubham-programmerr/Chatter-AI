...
router.post('/', authMiddleware, async (req, res) => {
  try {
    console.log('✨ Creating room:', { userId: req.userId, isPrivate: req.body.isPrivate });
    ...
    console.log('✅ Room created:', { roomId: room._id, isPrivate: room.isPrivate });
...
router.post('/:id/join', authMiddleware, async (req, res) => {
  try {
    console.log('📍 Join room attempt:', { roomId: req.params.id, userId: req.userId });
    ...
    console.log('🏠 Room found:', { name: room.name, isPrivate: room.isPrivate, ownerId: room.owner?._id });

    // Check if user is the owner
    const isOwner = room.owner && room.owner._id.toString() === req.userId;
    console.log('👤 Is owner?', isOwner);

    // If private and NOT owner, reject
    if (room.isPrivate && !isOwner) {
      console.log('🔒 Private room - owner only');
...
// Update room (change private/public)
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const { isPrivate, description, name } = req.body;
