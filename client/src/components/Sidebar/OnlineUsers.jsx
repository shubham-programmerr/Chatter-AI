  ...
  return (
    <div className="p-2 md:p-3 bg-gradient-to-b from-white to-gray-50">
      <div className="flex items-center gap-1.5 md:gap-2 mb-2 md:mb-2">
        ...
        <h3 className="font-bold text-gray-800 text-xs md:text-xs">
          Online ({onlineUsers.length})
        </h3>
      </div>
      {onlineUsers.length === 0 ? (
        <p className="text-gray-500 text-xs md:text-xs italic">No users</p>
      ) : (
        <div className="space-y-1 md:space-y-1">
          {onlineUsers.map((user) => (
            <div key={user._id} className="flex items-center gap-1.5 md:gap-2 text-xs md:text-xs group cursor-pointer">
