        ...
        <div className="space-y-2">
          {onlineUsers.map((user) => (
            <div key={user._id} className="flex items-center gap-2 md:gap-3 text-base md:text-sm group cursor-pointer">
              ...
              <span className="truncate text-gray-700 font-medium group-hover:text-blue-600 transition flex-1">{user.username}</span>
              <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-green-500 flex-shrink-0 animate-pulse"></div>
            </div>
          ))}
        </div>
      )}
