...
const RoomList = ({ rooms, currentRoomId, onJoinRoom }) => {
  ...
  const { user } = useAuth();
  const [displayedRooms, setDisplayedRooms] = React.useState(rooms.slice(0, 8));
  ...
  React.useEffect(() => {
    if (showAll) {
      ...
      setDisplayedRooms(rooms.slice(0, 8));
    }
  }, [rooms, showAll]);
  ...
  return (
    <div className="space-y-1 md:space-y-0.5">
      <h3 className="text-sm md:text-xs font-bold text-gray-600 px-2 md:px-3 pt-2 md:pt-3 uppercase tracking-wider">Rooms {rooms.length > 0 && `(${rooms.length})`}</h3>
      {rooms.length === 0 ? (
        ...
        <div>
          ...
          {rooms.length > 8 && (
            <button
              ...
              className="w-full text-center px-2 md:px-2.5 py-2 md:py-1.5 text-xs md:text-xs text-blue-600 hover:text-blue-700 font-semibold rounded-md hover:bg-blue-50 transition"
            >
              {showAll ? '➖ Show Less' : `➕ Show More (${rooms.length - 8})`}
            </button>
          )}
        </div>
      )}
